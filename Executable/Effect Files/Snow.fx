//=============================================================================
// Snow.fx by Elinor Townsend
// Based on rain.fx by Frank Luna (C) 2008 All Rights Reserved.
//
// Snow particle system.  Particles are emitted directly in world space.
//=============================================================================
...
cbuffer cbFixed
{
	...
	float3 accelW = {0.025f, -0.4f, -0.05f};
...
{	
	...
	{	
		...
		if( gIn[0].age > 0.01f )
		{
			for(int i = 0; i < 5; ++i)
			{
				// Spread snowflakes out above the camera
				float3 posRandom = 50.0f*RandVec3((float)i/5.0f);
				float3 velRandom = RandUnitVec3(0.0f);
				posRandom.y = 25.0f;
			
				Particle p;
				p.initialPosW = emitPosW.xyz + posRandom;
				p.initialVelW = velRandom * 1.5f;
				p.sizeW       = float2(0.05f, 0.05f);
				p.age         = 0.0f;
				p.type        = PT_FLARE;
